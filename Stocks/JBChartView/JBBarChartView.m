...
CGFloat static const kJBBarChartViewPopOffset = 0.0f; // used to offset bars for 'pop' animations
...
@property NSMutableDictionary *prevYDict;
...
- (void)construct
{
    ...
    self.prevYDict = [[NSMutableDictionary alloc] init];
}
...
- (void)reloadData
{
    ...
    dispatch_block_t createBars = ^{
        ...
        for (NSNumber *key in [[self.chartDataDictionary allKeys] sortedArrayUsingSelector:@selector(compare:)])
        {
            ...
            //NSLog(@"original height=%f", [[self.chartDataDictionary objectForKey:key] floatValue]);
            CGFloat height = [self normalizedHeightForRawHeight:[self.chartDataDictionary objectForKey:key]];
            //NSLog(@"normalized height=%f", height);
            
            CGFloat posY = self.bounds.size.height - self.footerView.frame.size.height;
            if (height < 0) {
                posY = posY/2;
            } else {
                posY = posY/2 - height;
            ...
            CGFloat extensionHeight = fabs(height) > 0.0 ? kJBBarChartViewPopOffset : 0.0;
            //CGFloat extensionHeight = 0.0;
            
            if ([self minimumValue] < 0) {
                barView.frame = CGRectMake(xOffset, posY, [self barWidth], fabs(height) + extensionHeight);
            } else {
                barView.frame = CGRectMake(xOffset, self.bounds.size.height - self.footerView.frame.size.height - height, [self barWidth], fabs(height) + extensionHeight);
            }

            
            NSLog(@"coordinate=(%f,%f, %f,%f)", barView.frame.origin.x, barView.frame.origin.y, barView.frame.size.width, barView.frame.size.height);
            
            [mutableBarViews addObject:barView];
			
            
            [self.prevYDict setObject:[NSString stringWithFormat:@"%f", barView.frame.origin.y] forKey:[NSString stringWithFormat:@"%d", viewIndex]];
            ...
            // Add bar text
            if ([[self.chartDataDictionary allKeys] count] <= 10) {
                if ([self.dataSource respondsToSelector:@selector(barChartView:barTextAtIndex:)]) {
                    
                    UILabel *textLabel = [self.dataSource barChartView:self barTextAtIndex:index];
                    
                    if (barView.frame.origin.y == 140.0 && barView.frame.size.height > 0.0) {
                        [textLabel setFrame:CGRectMake(barView.frame.origin.x, barView.frame.origin.y - 10.0, barView.frame.size.width, 10.0)];
                    } else {
                        [textLabel setFrame:CGRectMake(barView.frame.origin.x, 140.0, barView.frame.size.width, 10.0)];
                    }
...
- (CGFloat)normalizedHeightForRawHeight:(NSNumber*)rawHeight
{
    ...
    if (minHeight >= 0) {
        ...
        return ((value - 0) / (maxHeight - minHeight)) * [self availableHeight];
    }
}
    ...
    if ([self.barViews count] > 0)
    {
        if (animated)
        {
            ...
            for (UIView *barView in self.barViews)
            {
                ...
                [UIView animateWithDuration:kJBBarChartViewStateAnimationDuration delay:(kJBBarChartViewStateAnimationDuration * 0.5) * index options:UIViewAnimationOptionBeginFromCurrentState animations:^{
                    ...
                    barView.frame = CGRectMake(barView.frame.origin.x, [[self.prevYDict objectForKey:[NSString stringWithFormat:@"%d", viewIndex]] floatValue], barView.frame.size.width, barView.frame.size.height);
                    ...
                    [UIView animateWithDuration:kJBBarChartViewStateAnimationDuration delay:0.0 options:UIViewAnimationOptionBeginFromCurrentState animations:^{
                        if (state == JBChartViewStateExpanded)
                        {
                            //barView.frame = CGRectMake(barView.frame.origin.x, popOffset - barView.frame.size.height + kJBBarChartViewPopOffset, barView.frame.size.width, barView.frame.size.height);
                             barView.frame = CGRectMake(barView.frame.origin.x, [[self.prevYDict objectForKey:[NSString stringWithFormat:@"%d", viewIndex]] floatValue] + kJBBarChartViewPopOffset, barView.frame.size.width, barView.frame.size.height);
                        }
        ...
        else
        {
            ...
            for (UIView *barView in self.barViews)
            {
                ...
                if (state == JBChartViewStateExpanded)
                {
                    NSLog(@"%f", [[self.prevYDict objectForKey:[NSString stringWithFormat:@"%d", viewIndex]] floatValue]);
